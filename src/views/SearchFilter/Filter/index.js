...
function removeOptionToUrl(url, key = '', value = '') {
    ...
    queryList.forEach((query) => {
        let k = Object.keys(query)[0];
        if (k === key) {
            query[k] = query[k].replace(new RegExp(`(${value});?|;?(${value})`, 'gi'), '');
            isChanged = true;
        }
    });
...
function FilterResult() {
    ...
    const onChecked = (sub, query, key) => {
        ...
        if (index === -1) {
            ...
            // thêm tag mới
            setTagList([...tagList, { key, query, to, title, color: helpers.randomColor() }]);
            // thay đổi url
            setUrl(addOptionToUrl(url, query.slice(0, query.length - 1), to));
        }
        // xoá key hiện tại đã chọn
        else {
            const newActiveList = [...activeList.slice(0, index), ...activeList.slice(index + 1, activeList.length)];
            setActiveList([...newActiveList]);
            setTagList([...tagList.slice(0, index), ...tagList.slice(index + 1, tagList.length)]);
    ...
    const onCloseTag = (key, query, to) => {
        ...
        const newTagList = [...tagList.slice(0, index), ...tagList.slice(index + 1, tagList.length)];
        setTagList([...newTagList]);
        setActiveList([...activeList.slice(0, index), ...activeList.slice(index + 1, activeList.length)]);
    ...
    function renderFilterOption(type) {
        if (type < 0) return;
        const list = constants.FILTER_OPTION_LIST.find((item) => item.key === type).data;
        return (
            list &&
            list.map((item, index) => (
                            ...
                            return (
                                <Button
                                    ...
                                    className={`bor-rad-4 m-r-8 ${
                                        activeList.findIndex((value) => value === key) === -1 ? '' : 'filter-active-btn'
                                    }`}
    ...
    return (
        <div className="container" style={{ minHeight: '100vh' }}>
            ...
            {/* loading */}
            {isLoading ? (
                <Spin className="trans-center" tip="Đang tìm kiếm sản phẩm phù hợp ..." size="large" />
            ) : (
                <>
                    {/* Bộ lọc  */}
                    <div className="Filter-Options p-tb-16 bg-white bor-rad-8 box-sha-home">
                        <div className="list-active p-lr-16 p-b-8 d-flex justify-content-between">
                            ...
                            {tagList.length > 0 && (
                                <>
                                    <div className="d-flex align-i-center flex-wrap">{showTagList()}</div>
                                    <Button className="bor-rad-4" type="dashed" danger onClick={onCloseAll}>
                                        <b>Xoá tất cả</b>
                                    </Button>
                                </>
                            )}
