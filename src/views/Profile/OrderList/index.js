...
function OrderList() {
    ...
    const orderColumns = [
        {
            ...
            render: (orderCode, records) => (
                <Button type="link" onClick={() => setOrderDetails({ isOpen: true, orderId: records._id })}>
                    <b>{orderCode}</b>
                </Button>
            ),
        },
        ...
        },
        {
            ...
            render: (orderProd) => (
                <Link to={`/product/${orderProd.id}`}>
                    <Tooltip title={orderProd.name}>{helpers.reduceProductName(orderProd.name, 30)}</Tooltip>
                </Link>
            ),
        },
        {
            ...
            sorter: (a, b) => helpers.calTotalOrderFee(a) - helpers.calTotalOrderFee(b),
        },
        {
    ...
    return (
        <>
            ...
            {orderDetails.isOpen && (
                <OrderDetail orderId={orderDetails.orderId} onClose={() => setOrderDetails({ isOpen: false })} />
            )}
        </>
    );
}
