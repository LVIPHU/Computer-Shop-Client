...
const convertProductValue = (type = 0, product) => {
    ...
    switch (type) {
        ...
        // ram
        case 4:
            const newRamType = product.type === 0 ? 'DDR3' : product.type === 1 ? 'DDR3L' : 'DDR4';
        ...
        case 7:
            ...
            const newHPConnectionStd = convertHeadphoneConnectionStd(product.connectionStd);
            return {
                ...product,
        ...
        case 8:
            const newKBType =
                product.type === 0 ? 'Bàn phím thường' : product.type === 1 ? 'Bàn phím giả cơ' : 'Bàn phím cơ';
        ...
        case 14:
            const newResolution =
                product.resolution === 0 ? '720p' : product.resolution === 1 ? '1280 x 720' : '1920 x 1080';
            return {
                ...product,
...
const formatOrderDate = (date = Date.now(), flag = 0) => {
    ...
    return flag === 0 ? `${d}/${m}/${y}` : `${newDate.getHours()}:${newDate.getMinutes()} ${d}/${m}/${y}`;
};
...
const calTotalOrderFee = (order) => {
    ...
    const total =
        transportFee + (orderProd.price * numOfProd - (orderProd.price * numOfProd * orderProd.discount) / 100);
    return total;
};
