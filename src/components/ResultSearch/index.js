...
function ResultSearch(props) {
    ...
    const onFilterByPrice = () => {
        setIsLoading(true);
        const { from, to } = price;
        let newList = initList.filter((item) => item.price >= from && item.price <= to);
        // delay
        setTimeout(() => {
            setIsLoading(false);
            setList(newList);
            ...
            <Col span={24} className="sort-wrapper p-lr-16">
                <div className="sort p-tb-10 d-flex align-i-center">
                    ...
                    {sortButtons.map((item) => (
                        <Button
                            className={`${item.key === sortBtnActive ? 'sort-btn-active' : ''} m-4 bor-rad-4`}
                            key={item.key}
                            size="large"
            ...
            <Col span={24} className="Result-Search-list p-16">
                {!list || list.length === 0 ? (
                    <div className="trans-center d-flex flex-direction-column pos-relative">
                        <img className="not-found-product m-0-auto" src={productNotFoundUrl} />
                        <span className="font-size-16px m-t-8 t-center">Không sản phẩm nào được tìm thấy</span>
                    </div>
                ) : isLoading ? (
                    <Spin className="trans-center" tip="Đang cập nhật sản phẩm ..." size="large" />
                ) : (
