...
function ProductView(props) {
    const { className, name, price, avtUrl, discount, stock, action, height, maxWidth } = props;

    // set height cho các avt của sản phẩm
    useEffect(() => {
        document.querySelectorAll('.ant-card-cover').forEach((item) => (item.style.height = `${height / 2}px`));
    }, []);
    ...
    return (
        ...
        >
            {/* Tên sản phẩm */}
            <div className="font-size-16px m-b-10">{helpers.reduceProductName(name)}</div>
                    ...
                    <>
                        ...
                        {discount > 0 && (
                            <div>
                                ...
                                <span className="m-l-8 Product-View-price--discount">{discount}%</span>
                            </div>
                        )}
                    </>
                )}
            ...
            {stock <= 5 && stock > 0 && (
                <div className="Product-View-stock font-size-14px">chỉ còn {stock} sản phẩm</div>
            )}

            {/* Số lượng hàng còn, chỉ hiển thị khi còn ít hơn 5 */}
            {stock === 0 && <b className="Product-View-stock font-size-16px">Đang hết hàng</b>}

            {/* Các nút bấm thêm nếu có */}
            <div className="d-flex m-t-10 justify-content-end">{action.length > 0 && action.map((Item) => Item)}</div>
        </Card>
    );
}
